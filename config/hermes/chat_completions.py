...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        for raw_msg in messages:
            ...
            # 1. Strip internal non-standard fields
                ...
                if isinstance(tool_calls, list) and tool_calls:
                    ...
                    for tc in tool_calls:
                        if isinstance(tc, dict):
                            ...
                            cid = str(copied_tc.get("id") or "").strip()
                            if cid:
                                valid_ids.add(cid)
                            copied_tool_calls.append(copied_tc)
                    if copied_tool_calls:
                        msg["tool_calls"] = copied_tool_calls
                        last_assistant_tool_ids = valid_ids
                    else:
            ...
            # 3. Tool result message handling
                ...
                # Ensure tool content is always a valid string
                ...
                # If orphaned (not matching any tool_call from the prior assistant message),
                # convert to a user-role message so Ollama / OpenAI API does not reject with 500/400.
                if not tc_id or tc_id not in last_assistant_tool_ids:
                    msg["role"] = "user"
            ...
            # 4. User / System / other messages
            else:
                # If the immediately preceding message was a tool message, inject an intervening
                # assistant acknowledgment to prevent tool -> user role alternation violations.
                if sanitized and sanitized[-1].get("role") == "tool" and role == "user":
                    sanitized.append({
                        "role": "assistant",
        ...
        # 5. Tail validation: if the sequence ends in an assistant message with unclosed tool_calls,
        # close them with synthetic placeholders so Ollama never receives dangling trailing tool_calls.
        if sanitized and sanitized[-1].get("role") == "assistant":
            tail_calls = sanitized[-1].get("tool_calls")
            if isinstance(tail_calls, list) and tail_calls:
        ...
        # 6. User query presence invariant: Ollama / OpenAI validator requires at least one user-role message
        if not any(m.get("role") == "user" for m in sanitized):
            insert_idx = 1 if (sanitized and sanitized[0].get("role") == "system") else 0
            sanitized.insert(insert_idx, {
                "role": "user",
                "content": "Execute quantitative reconnaissance."
            })
