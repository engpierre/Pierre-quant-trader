"""
pierre_quant/forecasting/chronos_agent.py
Agent 06b: Amazon Chronos-Bolt Forecasting Engine (cuda:1 / optimized runtime).
"""
...
def get_chronos_pipeline(device: str = "cuda:1") -> Optional[Any]:
    global _CHRONOS_PIPELINE
    if _CHRONOS_PIPELINE is not None:
        return _CHRONOS_PIPELINE

    target_device = device if (torch.cuda.is_available() and torch.cuda.device_count() > 1 and "cuda" in device) else ("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    try:
        ...
        logger.warning(f"Unable to load Chronos pipeline: {e}")
        return None


class ChronosForecastingAgent:
    """Agent 06b: Amazon Chronos-Bolt probabilistic forecasting agent."""
    ...
    def forecast_trajectory(cls, ticker: str, period: str = "6mo") -> AgentExecutionPayload:
        ...
        if feed_payload.status != ExecutionStatus.SUCCESS or not feed_payload.candles:
            return AgentExecutionPayload(
                ...
                error_message=f"Live feed failed: {feed_payload.error_message}"
            )

        closes = np.array([c.close for c in feed_payload.candles if not np.isnan(c.close)], dtype=np.float32)
        if len(closes) < 10:
            return AgentExecutionPayload(
                agent_id=cls.AGENT_ID,
                ticker=clean_ticker,
        ...
        # Context padding / truncation to 128 bars
        if len(closes) > cls.CONTEXT_LEN:
            context_arr = closes[-cls.CONTEXT_LEN:]
        else:
            context_arr = np.pad(closes, (cls.CONTEXT_LEN - len(closes), 0), mode="edge")

        mean_vector: List[float] = []
        pipeline = get_chronos_pipeline(device=cls.DEVICE)

        if pipeline is not None:
            try:
                context_tensor = torch.tensor(context_arr, dtype=torch.float32)
                forecast = pipeline.predict(context_tensor, prediction_length=cls.HORIZON)
                mean_vector = [round(float(x), 4) for x in forecast[0].mean(dim=0)]
            except Exception as infer_err:
                logger.warning(f"Chronos inference exception: {infer_err}")

        # Deterministic momentum autoregression fallback if inference failed
        if not mean_vector:
        ...
        return AgentExecutionPayload(
            ...
            metrics={
                ...
                "input_context_bars": cls.CONTEXT_LEN,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Agent 06b: Chronos-Bolt Forecasting CLI")
