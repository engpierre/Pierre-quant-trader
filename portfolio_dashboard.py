...
# Injecting clean CSS without leading whitespace traps
st.markdown("""<style>
...
.metric-card { background: rgba(22, 27, 34, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 15px; backdrop-filter: blur(12px); display: flex; flex-direction: column; justify-content: space-between; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: transform 0.2s ease-in-out; }
...
.amber-pulse { border: 1px solid rgba(210, 153, 34, 0.8) !important; background-color: rgba(210, 153, 34, 0.15) !important; animation: pulse-amber 2s infinite; }
...
.typewriter { font-family: 'Courier New', Courier, monospace; color: #58a6ff; padding: 15px; border-left: 3px solid #58a6ff; background: rgba(13, 17, 23, 0.8); border-radius: 4px; box-shadow: inset 0 0 10px rgba(0,0,0,0.5); }
...
if len(watchlist) > 0:
    ...
    for item in watchlist:
        ...
        # Linear HTML initialization prevents markdown indentation interpretation errors
    ...
    with b_col1:
        if st.button("📥 Commit Position Update", use_container_width=True, type="primary"):
            if sel_ticker:
                ...
                st.warning("Please specify a valid asset symbol.")
        ...
        if st.button("❌ Purge Asset", use_container_width=True):
            if sel_ticker and sel_ticker != "NEW...":
                try:
                    ...
                    conn.commit()
                    conn.close()
                    st.warning(f"Position {sel_ticker} cleanly purged from database.")
                    time.sleep(1)
                    st.rerun()
                except Exception as e:
                    st.error(f"SQL Error: {e}")
...
if prompt := st.chat_input("Initiate direct sequence..."):
    ...
    with st.spinner("Compiling tactical briefing..."):
        try:
            ...
            verbal_report = f"Offline routing failed. System message: {str(e)}."
